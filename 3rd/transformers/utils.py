    ...
    def _greedy_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            auras_input_ids = input_ids[:, :-6]
            if auras:
                next_k_token_logits = outputs.logits[:, -7:, :]
                input_ids_len = input_ids.shape[1]

                for i in range(7):
                    single_position_logits = next_k_token_logits[:, i, :]
                    processed_logits = logits_processor(input_ids[:, :input_ids_len-6+i], single_position_logits)
                    next_token = torch.argmax(processed_logits, dim=-1)
                    auras_input_ids = torch.cat([auras_input_ids, next_token[:, None]], dim=-1)
